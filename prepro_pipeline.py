...
def butter_lowpass_filter(data, cutoff=10, fs=120, order=5):
    ...
    data_lp_zerophase = filtfilt(b, a, data, padtype="even")  # this is zero phase, applies the filter twice (doubles the original order?)
    return data_lp
...
n_SD = 3  # number of SD-s to cut from median (applied for the whole recording as one of the first steps)
...
print(np.size(data_dil2))
#plt.show()


# Interpolate over missing data

data_interp = interpolate(data_dil2, min_diff=3, padding=0, n_samples=1, method="lin")  # usually needs some adjusting
# data_interp2 = interpolate(data_interp, method="cubic")
print("Interpolation done.")
print(np.size(data_interp))

# nans_left = np.argwhere(np.isnan(data_interp))
# print(nans_left)
# print(len(nans_left))
# if nans_left[0] == 0:
#     time = time[(len(nans_left)):]

# plot things so far
matplotlib.pyplot.plot(time[0:len(data_interp)], data_interp, c="pink", label="interpolated")
plt.grid(True)
plt.legend(loc="upper left")
#plt.show()
...
med_filt_pldata = scipy.signal.medfilt(data_interp, filt_order)
matplotlib.pyplot.plot(time[0:len(med_filt_pldata)], med_filt_pldata, c="black", label="median filtered")
#plt.legend(loc="upper left")
#plt.show()

# NaN values in input results in all NaN values...
# interpolation took care of most of them, but we still have some around the edges of the data
# so we'll just skip those
pl_dia_low_pass = butter_lowpass_filter(med_filt_pldata[np.logical_not(np.isnan(med_filt_pldata))], cutoff=10, order=5)
matplotlib.pyplot.plot(time[0:len(pl_dia_low_pass)], pl_dia_low_pass, c="orange", label="10 Hz low pass")
...
# matplotlib.pyplot.plot(time, pldata_detrended, c="lightblue", label="Detrended")
# plt.legend(loc="lower left")
# plt.grid()
# plt.show()
